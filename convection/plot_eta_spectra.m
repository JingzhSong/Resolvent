...
N = 400; % wall-normal grid (400 for R6700 and R8900, 300 for R3300)
R = 6700; % Reynolds number
nc = 201; % omega grid
nkx = 100;% kx grid
nkz = 100;% kz grid
...
%% read data
uvwp = 'v';
path = ['H:/resolvent_convection/',wall,'/',uvwp,'/psd_N=',num2str(N),'nomega=',num2str(nc),'nkx=',num2str(nkx),'nkz=',num2str(nkz),'_R=',num2str(R),'_Cm=',num2str(Cm),'_Ck=',num2str(Ck),'_Cd=',num2str(Cd),eddy,'Ucom_final'];
load([path,'.mat'],'psd');
psd = real(psd);
...
A = (spec./(sum(spec,'all').*dkx.*domega))';

figure;
pcolor(KX,OMEGA,log10(A)); % 整体积分是1，归一化

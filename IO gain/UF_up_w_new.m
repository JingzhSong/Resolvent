...
%     vel_u = inv(Dr)*U(1:3*N,1);
%     vel_v = inv(Dr)*U(3*N+1:6*N,1);
%     vel_w = inv(Dr)*U(6*N+1:9*N,1);
%     
%     vel_u = inv(diag(w.^0.5))*vel_u(1:N,1)./i./kx;
%     vel_v = inv(diag(w.^0.5))*vel_v(1:N,1)./i./kx;
%     vel_w = inv(diag(w.^0.5))*vel_w(1:N,1)./i./kx;
    
    vel_u = inv(diag(w.^0.5))*U(1:N,1)./i./kx;
    vel_v = inv(diag(w.^0.5))*U(3*N+1:4*N,1)./i./kx;
    vel_w = inv(diag(w.^0.5))*U(6*N+1:7*N,1)./i./kx;
    ...
    uvwp = Hoo * V(:,1);
    p = uvwp(3*N+1:4*N,1) ./ eig .*Dl(3*N+1,3*N+1); % The same scale as v
